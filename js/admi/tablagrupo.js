...
var coleccionArchivos = db.ref().child("GrupoEsc");
...
coleccionArchivos.on("child_added", datos => {        
    dataSet = [datos.key, datos.child("Grupo_Letra").val()];
...
$("#tablagrupo").on("click", ".btnEditar", function() {    
    filaEditada = table.row($(this).parents('tr'));           
    let fila = $('#tablagrupo').dataTable().fnGetData($(this).closest('tr'));               
    let id = fila[0];
    console.log(id);
...
$("#tablagrupo").on("click", ".btnBorrar", function() {   
    filaEliminada = $(this);
    Swal.fire({
