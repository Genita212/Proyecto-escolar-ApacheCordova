...
var coleccionArchivos = db.ref().child("YearEsc");
     
var dataSet = [];//array para guardar los valores de los campos inputs del form
var table = $('#tablay').DataTable({
            pageLength : 5,
...
coleccionArchivos.on("child_added", datos => {        
    dataSet = [datos.key, datos.child("Año_Escolar").val()];
...
$("#tablay").on("click", ".btnEditar", function() {    
    filaEditada = table.row($(this).parents('tr'));           
    let fila = $('#tablay').dataTable().fnGetData($(this).closest('tr'));               
    let id = fila[0];
    console.log(id);
...
$("#tablay").on("click", ".btnBorrar", function() {   
    filaEliminada = $(this);
    Swal.fire({
    ...
    if (result.value) {
        let fila = $('#tablay').dataTable().fnGetData($(this).closest('tr'));            
        let id = fila[0];            
        db.ref(`YearEsc/${id}`).remove()
        Swal.fire('¡Eliminado!', 'El año ha sido eliminado.','success')
    }
    })        
});  
