...
$(document).ready(function() {
    ...
    var coleccionArchivos = db.ref().child("AdministradorEsc");
         
    var dataSet = [];//array para guardar los valores de los campos inputs del form
    var table = $('#tablaAdmi').DataTable({
                pageLength : 5,
    ...
    coleccionArchivos.on("child_added", datos => {        
        dataSet = [datos.key, datos.child("NumEmpleado").val(), datos.child("Nombre").val(), datos.child("Apellidos").val(), datos.child("CorreoElectronico").val()];
    ...
    $('form').submit(function(e){                         
        ...
        data = {NumEmpleado:codigo, Nombre:nombre, Apellidos:apellidos, CorreoElectronico:correo};             
        actualizacionData = {};
    ...
    $("#tablaAdmi").on("click", ".btnEditar", function() {    
        filaEditada = table.row($(this).parents('tr'));           
        let fila = $('#tablaAdmi').dataTable().fnGetData($(this).closest('tr'));               
        let id = fila[0];
        console.log(id);
        ...
        let correo = ($(this).closest('tr').find('td:eq(3)').text());        
        $('#id').val(id);        
        $('#codigo').val(codigo);
        $('#nombre').val(nombre);                
    ...
    $("#tablaAdmi").on("click", ".btnBorrar", function() {   
        filaEliminada = $(this);
        Swal.fire({
        title: '¿Está seguro de eliminar al administrador?',
        ...
        if (result.value) {
            let fila = $('#tablaAdmi').dataTable().fnGetData($(this).closest('tr'));            
            let id = fila[0];            
            db.ref(`AdministradorEsc/${id}`).remove()
