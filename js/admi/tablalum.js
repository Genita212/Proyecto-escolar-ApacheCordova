...
coleccionArchivos.on("child_added", datos => {        
    dataSet = [datos.key, datos.child("Matricula").val(), datos.child("Nombre").val(), datos.child("Apellidos").val(), datos.child("CURP").val(), datos.child("CorreoElectronico").val()];
...
$('form').submit(function(e){                         
    ...
    data = {Matricula:codigo, Nombre:nombre, Apellidos:apellidos, CURP:curp, CorreoElectronico:correo};             
    actualizacionData = {};
...
$("#tablalum").on("click", ".btnEditar", function() {    
    filaEditada = table.row($(this).parents('tr'));           
    let fila = $('#tablalum').dataTable().fnGetData($(this).closest('tr'));               
    let id = fila[0];
    console.log(id);
    ...
    let correo = ($(this).closest('tr').find('td:eq(4)').text());        
    $('#id').val(id);        
    $('#codigo').val(codigo);
    $('#nombre').val(nombre);                
...
$("#tablalum").on("click", ".btnBorrar", function() {   
    filaEliminada = $(this);
    Swal.fire({
