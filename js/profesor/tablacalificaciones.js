...
var coleccionArchivos = db.ref().child("CalificacionEsc/");
...
coleccionArchivos.on("child_added", datos => {        
    dataSet = [datos.key, datos.child("Matricula").val(), datos.child("Clave_modulo").val(), datos.child("Calificacion").val()];
...
$('form').submit(function(e){                         
    ...
    let calif = $.trim($('#calif').val());                        
     let idFirebase = id;        
    if (idFirebase == ''){                      
        idFirebase = coleccionArchivos.push().key;
    };                
    data = {Matricula:matricula, Clave_modulo:modulo,  Calificacion:calif};             
    actualizacionData = {};
...
$("#tablacalif").on("click", ".btnEditar", function() {    
    ...
    let calif = $(this).closest('tr').find('td:eq(2)').text();    
    $('#id').val(id);        
